...
export function RecognizerResult(props: RecognizerResultProps) {
  ...
  const renderEntity = (label: string, value: string = '') => {
  ...
  const renderResults = (results: RecognizerData) => {
    ...
    if (hasNoEntities) {
      ...
      return results.map((entity: DocumentField) => renderEntity(entity.kind, entity.content));
    ...
    return Object.entries(results).map(([key, entity]: [string, DocumentField]) =>
      ...
      renderEntity(key, getValueFromEntity(entity))
...
export default withTranslation('common')(RecognizerResult);